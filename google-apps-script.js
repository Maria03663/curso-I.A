...
function buildBienvenidaHTML(data) {
  var n = esc(data.fullName), e = esc(data.email), u = esc(data.username) || e, c = esc(data.course) || '—', p = esc(data.password) || '—';
  return '<!DOCTYPE html><html><head><meta charset="UTF-8"></head>'
    ...
    + '<h1 style="margin:0 0 8px;font-size:22px;font-weight:700;color:#1E293B">¡Bienvenido, ' + n + '!</h1>'
    ...
    + '<tr><td style="font-size:14px;color:#64748B;padding-bottom:6px">🔐 Contraseña</td><td style="font-size:14px;color:#1E293B;font-weight:700;padding-bottom:6px;text-align:right;font-family:monospace;letter-spacing:1px">' + p + '</td></tr></table></div>'
    + '<p style="margin:20px 0 0;font-size:14px;color:#64748B;line-height:1.7">Ya podés iniciar sesión con tus credenciales y explorar el contenido del curso.</p>'
    + '<div style="margin:24px 0 0;text-align:center">'
    + '<a href="' + SITE_URL + '/perfil.html" style="display:inline-block;padding:14px 36px;background:#8B5CF6;color:#FFFFFF;text-decoration:none;border-radius:10px;font-weight:700;font-size:15px">Ir a Mi Perfil →</a></div>'
    + '</td></tr>'
...
function buildAceptacionHTML(data) {
  var n = esc(data.fullName), e = esc(data.email), c = esc(data.course) || 'CampusIA', p = esc(data.password) || '—';
  return '<!DOCTYPE html><html><head><meta charset="UTF-8"></head>'
    + '<body style="margin:0;padding:0;background:#F3F4F6;font-family:Segoe UI,Arial,Helvetica,sans-serif">'
    + '<table width="100%" cellpadding="0" cellspacing="0" style="background:#F3F4F6;padding:40px 0"><tr><td align="center">'
    + '<table width="560" cellpadding="0" cellspacing="0" style="background:#FFFFFF;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08)">'
    ...
    + '<tr><td style="padding:32px">'
    + '<h1 style="margin:0 0 8px;font-size:22px;font-weight:700;color:#1E293B">🎉 ¡Felicitaciones, ' + n + '!</h1>'
